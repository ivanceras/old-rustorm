...
fn main(){
    ...
    let pool = Arc::new(Mutex::new(Pool::init()));
    for i in 0..10{
        let pool = pool.clone();
        thread::spawn(move || {
            println!("spawning thread {}", i);
            let mut pool = pool.lock().unwrap();
            println!("At first, there are {} free connection", pool.total_free_connections());
            let db = pool.get_db_with_url(&url);
            match db{
                    Ok(db) => {
                    show_product(db.as_ref());//borrow a database
                    //thread::sleep_ms(500*i);
                    println!("And then there are {} free connection", pool.total_free_connections());
                    pool.release(db);//borrow has ended, release it
                    println!("Finally {} free connection", pool.total_free_connections());
                 }
                Err(e) => {
