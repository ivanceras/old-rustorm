...
If you don't have a sample database schema, use the provide schema_dump in `./scripts/bazaar_v6_dump_schema.sql`

```bash
psql -U postgres -h localhost -W -d <bazaar_v5> -f ./scripts/bazaar_v6_all.sql
```
...
### example:
This is an example table `product` located in `./scripts/bazaar_v6_dump_schema.sql`
```sql
