...
impl <'a>EntityManager<'a>{
    ...
    pub fn get_all<T>(&self)->Result<Vec<T>, DbError> where T : IsTable + IsDao{
        let table = T::table();
...
/// 
    ...
    pub fn insert_with_ignore_columns<T>(&self, dao:Dao, ignore_columns:Vec<&str>)->Result<T, DbError>
        where T: IsTable + IsDao {
        let table = T::table();
        let mut q = Query::insert();
        q.into_table(&table.complete_name());
